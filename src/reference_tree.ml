...
exception Nonexistent_child of string

module type RefTreeOrd = sig
    type t
    val compare : t -> t -> int
end
module RefTreeOrd = struct
    type t = Reference_tree.t
    let compare a b =
        Util.lexical_numeric_compare (Vytree.name_of_node a) (Vytree.name_of_node b)
end
module ChildrenS = Set.Make(RefTreeOrd)
...
let rec ref_tree_union s t =
    ...
    let child_of_union s t c =
        ...
        | None, Some _ -> t
        | Some u, Some v ->
                (* Names are equal; if the data is different, replace the node *)
                let t =
                    if Vytree.data_of_node u <> Vytree.data_of_node v then Vytree.replace t v
                else t
                in
                    Vytree.replace t (ref_tree_union u v)
