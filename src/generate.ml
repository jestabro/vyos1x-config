...
(*
Need a refactor for addons:
    interface_definitions_to_cache
    merge_reference_tree_cache
    reference_tree_cache_to_json
    [ load_reference_tree to vyconfd ]
*)

let interface_definitions_to_cache from_dir cache_name =
    let ref_tree_result =
    ...
    in
    I.write_internal ref_tree cache_name

let reference_tree_cache_to_json cache_name render_file =
    let ref_tree =
        I.read_internal cache_name
    in
...
let merge_reference_trees dir primary_name result_name =
    let file_arr = Sys.readdir dir in
    let file_list' = Array.to_list file_arr in
    let file_list =
        List.filter (fun x -> x <> primary_name && x <> result_name) file_list' in
    let file_path_list =
        List.map (FilePath.concat dir) file_list in
    let primary_tree = I.read_internal (FilePath.concat dir primary_name) in
    let ref_trees = List.map I.read_internal file_path_list in
    match ref_trees with
    | [] ->
        I.write_internal primary_tree (FilePath.concat dir result_name)
    | _ ->
        let f _ v = v in
        let res = List.fold_left (fun p r -> Tree_alg.RefAlg.tree_union r p f) primary_tree ref_trees in
        I.write_internal res (FilePath.concat dir result_name)
