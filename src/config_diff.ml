...
(* tree diff algorithm: a scaffolding is constructed functionally, with a
   function of type diff_func called at bifurcation points; in the
   implementation below, the diff_func instance imperatively builds the
   resulting difference trees. The design of the scaffolding uses the idea
   of matching on pairs of (node opt) from
   https://github.com/LukeBurgessYeo/tree-diff
   The construciton of the full scaffold is unnecessary for the diff_func
   results, and is maintained for experiments and further anaylsis.
*)
...
let rec clone_path ?(with_children=true) old_root new_root path_done path_remaining =
    ...
    | [] | [_] ->
        ...
        if with_children then
            Vytree.insert ~children:(children_of old_node) new_root path_total (data_of old_node)
        else
            Vytree.insert new_root path_total (data_of old_node)
    | name :: names ->
