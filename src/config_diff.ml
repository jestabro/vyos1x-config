...
let config_diff (rt : Reference_tree.t) ?(recurse=true) (path : string list) (Diff_show res) (m : change) =
    ...
    (* the only subtlety in all this is the bookkeeping of closing open
       braces at correct level: the record field open_blocks is a list of
       open paths ordered by reverse inclusion, which are closed when the
       path being passed to config_diff no longer contains that element
     *)
